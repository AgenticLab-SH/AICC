...
import { guidanceStatus } from './adapters/guidance-status.mjs';
...
export async function collectStatus(options = {}) {
  ...
  const [accounts, ocxAccounts, ocx, portal, guidance, agents, workspace, tools] = await Promise.all([
    ...
    Promise.resolve((options.guidanceStatus ?? guidanceStatus)(options.guidance)),
  ...
  const components = [accounts, ocxAccounts, ocx, portal, guidance, {
