...
# the box-drawing and status glyphs used throughout the output, which raises
# UnicodeEncodeError mid-print. Re-encode our own streams as UTF-8 instead of
# stripping the characters, so output looks the same on every platform.
for _stream in (sys.stdout, sys.stderr):
    try:
        _stream.reconfigure(encoding="utf-8", errors="replace")
    except (AttributeError, OSError, ValueError):
        # Redirected or already-wrapped streams may not support reconfigure;
        # printing must never fail because of a console setting.
        pass
...
# Optional integrations live outside this tool's core. Paths are resolved from
# the user's own config so nothing here is tied to one machine or checkout.
CM_CONFIG_HINT = f"설정 파일 {cm_integrations.CONFIG_FILE}"
...
SHARED_ITEMS = [
    ...
    ".codex-global-state.json", "installation_id", "models_cache.json",
...
def resolve_account_selector(selector: str | None, accounts: list[str] | None = None, quiet: bool = False) -> str | None:
    ...
    user-defined account_id prefix because email filenames can change.
    """
    if selector is None:
        return None
    if accounts is None:
...
def _wait_codex_app_exit(timeout: int = 10):
    """Wait until the default App instance is gone, with spinner."""
    spinner = "⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏"
    for i in range(timeout * 4):
        if not _is_codex_app_running():
            return
        s = spinner[i % len(spinner)]
        print(f"  {colored('[1/3]', C.GRAY)} {colored(s, C.YELLOW)} App 종료 대기 중...", end="\r")
        sys.stdout.flush()
        time.sleep(0.25)
    raise RuntimeError(f"Codex App이 {timeout}초 안에 종료되지 않았습니다.")
...
def thread_home_activity(name: str, home: Path) -> dict:
    ...
    return {
        "active": bool(profile_pids or cli_pids or db_pids),
...
def sync_thread_index(
    ...
    *,
    ...
    if result.get("reason") == "home_active":
        ...
        print(f"  {colored('✓', C.GREEN)} {name}: 스레드 색인이 최신입니다.")
    else:
        print(
...
def launch_app_account(
    ...
    *,
    ...
    if sys.platform == "darwin":
        ...
        # The membership fingerprint intentionally ignores growth of an existing
        # rollout. A fresh App process is the safe boundary for rebuilding
        # sidebar metadata such as recency, title, and preview.
        synced = sync_thread_index(target, home=home, force=True)
        if synced is None or synced.get("reason") == "home_active":
    ...
    # The target home is inactive here, so a deliberate metadata rebuild keeps
    # the single-writer SQLite rule while refreshing continued conversations.
    synced = sync_thread_index(target, home=home, force=True)
    if synced is None or synced.get("reason") == "home_active":
        print("  App 실행을 중단했습니다. 스레드 색인을 먼저 안전하게 복구하세요.")
        return None
...
def tui():
    while True:
        ...
        print(f"    {colored('f', C.MAGENTA)}    GPT 계정 생성 서버 실행")
...
# === Stack watch ===
# Read-only by design. `cm stack` reports what is installed and how the local
# proxy is routing; applying an update is delegated to a command the user
# configures, because a safe upgrade needs backup/rollback steps that belong to
# the host, not to this tool.

OPENCODEX_NPM_PACKAGE = "@bitkyc08/opencodex"
OPENCODEX_HEALTH_URL = f"{cm_integrations.proxy_base_url()}/healthz"
...
def stack_status() -> dict:
    ...
    # An optional companion checkout (e.g. a self-hosted gateway). Absent by
    # default, so a fresh install reports nothing here instead of erroring.
    gateway_repo = cm_integrations.gateway_repo()
    gateway_port = cm_integrations.gateway_port()
    head = ""
    remote_head = ""
    dirty: list[str] = []
    if gateway_repo is not None:
        head = (_git_text(gateway_repo, ["rev-parse", "HEAD"]) or "").strip()
        remote = (_git_text(gateway_repo, ["ls-remote", "origin", "HEAD"]) or "").strip()
        remote_head = remote.split()[0] if remote else ""
        dirty = [
            line
            for line in (_git_text(gateway_repo, ["status", "--short"]) or "").splitlines()
            if line.strip()
        ]
    ...
    return {
        ...
        "gateway": {"configured": gateway_repo is not None,
                    "label": cm_integrations.gateway_label(),
                    "head": head[:12], "remote_head": remote_head[:12],
                    "behind": bool(head and remote_head and head != remote_head),
                    "dirty": len(dirty), "repo": gateway_repo,
                    "running": bool(gateway_port) and _port_is_listening(gateway_port)},
    }


KIRO_DESKTOP_CREDS_FILE = Path(
    os.environ.get(
        "CM_KIRO_DESKTOP_CREDS_FILE",
        str(Path.home() / ".aws" / "sso" / "cache" / "kiro-auth-token.json"),
    )
).expanduser()


def _kiro_cli_db_file() -> Path:
    """kiro-cli's local credential database, per-platform.

    Only ever opened read-only and compared by digest; the path differs by OS,
    so it is resolved at call time instead of being fixed to one layout.
    """
    override = os.environ.get("CM_KIRO_CLI_DB_FILE", "").strip()
    if override:
...
def show_stack_status():
    print()
    print(f"  {colored('◆ 스택 상태', C.CYAN, C.BOLD)}")
    status = stack_status()
    codex = status["codex"]
    ...
    print(
        f"      프록시    : {proxy_state}  ·  대시보드 "
        f"{cm_integrations.proxy_base_url()}/ (ocx gui)"
    )
    ...
    # Kiro is an optional provider of the proxy; only report on it once a Kiro
    # login actually exists, so a Codex-only install stays quiet.
    source = kiro_credential_source()
    if source != "unknown":
        source_label = {
            "desktop": (colored("✓", C.GREEN), "Kiro Desktop 로그인 (전체 모델)"),
            "kiro-cli": (colored("!", C.YELLOW), "kiro-cli 로그인 — 상위 모델 제한. "
                                                 "KIRO_CREDS_FILE 설정 후 ocx login kiro"),
            "detached": (colored("!", C.YELLOW), "어느 로컬 로그인과도 일치하지 않음(자체 갱신됨)"),
            "none": (colored("○", C.GRAY), "kiro 로그인 없음 — ocx login kiro"),
        }.get(source, (colored("?", C.GRAY), "확인 불가"))
        print(f"  {source_label[0]} {'kiro 자격':<13} {source_label[1]}")

    label = gateway["label"]
    if not gateway["configured"]:
        pass  # No companion checkout configured; nothing to report.
    elif gateway["head"]:
        mark = colored("!", C.YELLOW) if gateway["behind"] else colored("✓", C.GREEN)
        state = "원격과 다름" if gateway["behind"] else "최신"
        running = "실행 중" if gateway["running"] else "미실행"
        print(f"  {mark} {label:<13} {gateway['head']} → {gateway['remote_head']}  {running} · {state}")
        if gateway["dirty"]:
            print(
                f"      {colored('!', C.YELLOW)} 로컬 변경 {gateway['dirty']}개 — "
                "업데이트가 거부됩니다(의도된 보호)."
            )
    else:
        print(f"  {colored('!', C.YELLOW)} {label} 클론을 찾지 못했습니다: {gateway['repo']}")

    stack_cmd = cm_integrations.stack_update_command()
    if stack_cmd and (codex["behind"] or ocx["behind"] or (gateway["running"] and gateway["behind"])):
        print()
        print(f"  적용: {colored('cm stack apply', C.CYAN)}")
    elif codex["behind"] or ocx["behind"]:
        print()
        print(f"  적용: {colored('cm update', C.CYAN)} (codex CLI) · npm i -g {OPENCODEX_NPM_PACKAGE} (opencodex)")
    return status


def apply_stack_update():
    # No built-in upgrade procedure: a safe one needs host-specific backup and
    # rollback, so this only runs a command the user opted into.
    command = cm_integrations.stack_update_command()
    if not command:
        print()
        print(f"  {colored('!', C.YELLOW)} 스택 업데이트 명령이 설정되지 않았습니다.")
        print(f"  {CM_CONFIG_HINT} 또는 CM_STACK_UPDATE_COMMAND 환경변수에 지정하세요.")
        print(f"  개별 업데이트: cm update (codex CLI) · npm i -g {OPENCODEX_NPM_PACKAGE}")
        return
    executable = shutil.which(command[0]) or command[0]
    if not Path(executable).exists() and shutil.which(command[0]) is None:
        print(f"  {colored('✗', C.RED)} 실행 파일을 찾지 못했습니다: {command[0]}")
        return
    print()
    print(f"  {colored('◆ 스택 업데이트 실행', C.CYAN, C.BOLD)} ({command[0]})")
    proc = subprocess.run([executable, *command[1:]], check=False)
    if proc.returncode == 0:
        print(f"  {colored('✓', C.GREEN)} 완료. 상태를 다시 확인하세요: cm stack")
    else:
        print(f"  {colored('✗', C.RED)} 실패(exit={proc.returncode}). 도구가 백업에서 롤백을 시도합니다.")
...
# === First-run setup ===
# One command that gets a fresh machine to a working state: check prerequisites,
# install or upgrade the proxy, report the account mode without changing OCX-owned
# provider state, and import an existing login. Anything that needs a browser, an app
# store or an administrator is reported with the exact next step instead of being
# silently attempted.

# Store IDs and cask names change; these are convenience shortcuts only, and the
# download page is always shown so a stale identifier never blocks anyone.
CODEX_APP_INSTALL_HINTS = {
    "darwin": (
        "https://chatgpt.com/download",
        ["brew", "install", "--cask", "chatgpt"],
    ),
    "win32": (
        "https://chatgpt.com/download (또는 Microsoft Store에서 ChatGPT 검색)",
...
def _setup_line(ok: bool | None, label: str, detail: str) -> str:
    mark = colored("✓", C.GREEN) if ok else (colored("!", C.YELLOW) if ok is None else colored("✗", C.RED))
    return f"  {mark} {label:<16} {detail}"


def _install_opencodex(upgrade: bool) -> tuple[bool, str]:
    npm = _npm_executable()
    if not npm:
        return False, "npm이 없습니다. Node.js 18+ 설치 후 다시 실행하세요."
    code, text = _run_text_command(
        [npm, "install", "-g", f"{OPENCODEX_NPM_PACKAGE}@latest"], timeout=900
    )
    if code != 0:
        tail = (text or "").strip().splitlines()[-1:] or [""]
        return False, f"설치 실패: {tail[0]}"
    return True, "업그레이드 완료" if upgrade else "설치 완료"


def proxy_is_healthy() -> bool:
    """Whether the local proxy answers its health endpoint."""
    try:
        with urlopen(OPENCODEX_HEALTH_URL, timeout=5) as response:
...
def app_routing_target() -> str | None:
    """The base URL the Codex app/CLI is configured to send model traffic to.

    Read from the app home's own config.toml, because that file — not the
    proxy's config — decides whether the desktop app actually reaches the proxy.
    """
    ...
    for line in text.splitlines():
        stripped = line.strip()
        if stripped.startswith("#") or "openai_base_url" not in stripped:
            continue
...
def resync_account_configs() -> int:
    """Re-render every account's config.toml from the app-home master.

    Each isolated home holds a real config file, so a routing change written to
    `~/.codex/config.toml` (for example by `ocx start`) does not reach existing
    accounts until their copies are re-rendered. Returns the number updated.
    """
    updated = 0
    for name in list_accounts():
        ...
        if not home.is_dir():
            continue
        if sync_account_config(home):
            updated += 1
    return updated


def setup_command(args: list[str]):
    apply_changes = not any(arg in ("--check", "--dry-run", "-n") for arg in args)
    print()
    title = "◆ cm 초기 설정" if apply_changes else "◆ cm 초기 설정 점검 (변경 없음)"
    print(f"  {colored(title, C.CYAN, C.BOLD)}")
    ensure_dirs()

    # 1. Python is implied (we are running). Check npm, which everything else needs.
    npm = _npm_executable()
    print(_setup_line(bool(npm), "npm", npm or "없음 — Node.js 18+ 설치가 필요합니다"))

    # 2. Codex CLI.
    codex_version = _codex_command_version()
    print(_setup_line(bool(codex_version), "codex CLI",
                      f"{codex_version or '없음'}  ({_codex_command()})"))

    # 3. Desktop app. Installing it needs a store/browser, so only report how.
    app_exe = find_codex_desktop_exe()
    if app_exe:
        print(_setup_line(True, "데스크톱 앱", str(app_exe)))
    else:
        hint, command = CODEX_APP_INSTALL_HINTS.get(sys.platform, ("https://chatgpt.com/download", None))
        print(_setup_line(None, "데스크톱 앱", "없음 (CLI만으로도 cm은 동작합니다)"))
        print(f"      설치: {hint}")
        if command and shutil.which(command[0]):
            print(f"      또는: {' '.join(command)}")
        if apply_changes and not codex_version and npm:
            # No app and no CLI means nothing to authenticate with; the CLI is
            # the one piece that can be installed non-interactively.
            print(f"      codex CLI 설치 중… (npm i -g {CODEX_NPM_PACKAGE})")
            code, text = _install_latest_codex_cli()
            ok = code == 0
            print(_setup_line(ok, "codex CLI", "설치 완료" if ok else "설치 실패"))

    # 4. Proxy: optional. Install or upgrade only when asked to apply.
    ocx_path = shutil.which("ocx")
    installed = None
    if ocx_path:
        code, text = _run_text_command([ocx_path, "--version"], timeout=30)
        installed = _version_from_text(text) if code == 0 else None
    latest = _npm_latest_version(OPENCODEX_NPM_PACKAGE)
    if not ocx_path:
        if apply_changes and npm:
            print(f"      opencodex 설치 중… (npm i -g {OPENCODEX_NPM_PACKAGE})")
            ok, detail = _install_opencodex(upgrade=False)
            print(_setup_line(ok, "opencodex", detail))
        else:
            print(_setup_line(None, "opencodex", f"미설치 (선택) — npm i -g {OPENCODEX_NPM_PACKAGE}"))
    elif latest and _behind(installed, latest):
        if apply_changes:
            print(f"      opencodex {installed} → {latest} 업그레이드 중…")
            ok, detail = _install_opencodex(upgrade=True)
            print(_setup_line(ok, "opencodex", detail))
        else:
            print(_setup_line(None, "opencodex", f"{installed} → {latest} 업그레이드 가능"))
    else:
        print(_setup_line(True, "opencodex", f"{installed or '?'} 최신"))

    # 5. Account mode belongs to OCX. Report it, but never rewrite it from cm.
    if shutil.which("ocx"):
        mode = proxy_codex_account_mode()
        detail = {
            "pool": "pool — OCX가 새 작업의 계정을 선택",
            "direct": "direct — 호출 App/CLI의 계정을 그대로 사용",
        }.get(mode, "확인 불가")
        print(_setup_line(mode in {"pool", "direct"}, "account-mode", detail))

    # 6. Bring the existing login into cm's store so `cm status` is not empty.
    accounts = list_accounts()
    app_auth = (APP_CODEX_HOME / "auth.json").exists()
    if accounts:
        print(_setup_line(True, "계정", f"{len(accounts)}개 등록됨"))
    elif app_auth and apply_changes:
        print("      기존 로그인을 가져오는 중…")
        import_app_auth()
        print(_setup_line(bool(list_accounts()), "계정", f"{len(list_accounts())}개 등록됨"))
    elif app_auth:
        print(_setup_line(None, "계정", "미등록 — cm import-app으로 기존 로그인 가져오기"))
    else:
        print(_setup_line(None, "계정", "로그인 없음 — cm add로 로그인하세요"))

    # 7. The part that decides whether the desktop app can actually use proxy
    #    models: the proxy must be running AND the app's own config must point at
    #    it. Reporting these separately makes a half-configured state obvious.
    # Ordered by dependency: log in first, then start the proxy (which injects the
    # app routing on startup). Collected per-check, sorted before printing.
    next_steps: set[str] = set()
    if shutil.which("ocx"):
        running = proxy_is_healthy()
        print(_setup_line(running, "프록시",
                          f"{cm_integrations.proxy_base_url()} "
                          + ("응답함" if running else "응답 없음 — ocx start")))
        if not running:
            next_steps.add("ocx start")

        target = app_routing_target()
        if target:
            print(_setup_line(True, "앱 라우팅", f"{target} (config.toml)"))
        else:
            print(_setup_line(None, "앱 라우팅",
                              "네이티브 (프록시 모델 사용 불가) — ocx start가 주입합니다"))
            next_steps.add("ocx start")

        provider = proxy_default_provider()
        source = kiro_credential_source()
        if provider == "kiro" or source not in {"unknown", "none"}:
            label = {
                "desktop": "Kiro Desktop 로그인 (전체 모델)",
                "kiro-cli": "kiro-cli 로그인 — 상위 모델 제한",
                "detached": "로컬 로그인과 불일치(프록시가 자체 갱신)",
                "none": "로그인 없음 — ocx login kiro",
            }.get(source, "확인 불가")
            print(_setup_line(source in {"desktop", "detached"}, "kiro 자격", label))
            if source == "none":
                next_steps.add("ocx login kiro")
        else:
            print(_setup_line(None, "kiro 자격", "미설정 — ocx login kiro"))
            next_steps.add("ocx login kiro")

        # Existing accounts keep their own config copy, so a routing change in
        # the app home has to be propagated explicitly.
        if apply_changes and list_accounts():
            updated = resync_account_configs()
            if updated:
                print(_setup_line(True, "계정 라우팅", f"{updated}개 계정 config 갱신"))

    print()
    if next_steps:
        # Dependency order, not discovery order: a login must exist before the
        # proxy starts, or the proxy comes up with no Kiro credential.
        sequence = ["ocx start", "ocx login kiro"]
        ordered = [step for step in sequence if step in next_steps]
        print(f"  남은 단계: {colored(' → '.join(ordered), C.CYAN)}")
        print(f"  완료 후 다시: {colored('cm setup', C.CYAN)}")
    elif apply_changes:
        print(f"  다음: {colored('cm status', C.CYAN)} · 계정 추가 {colored('cm add', C.CYAN)} · 진단 {colored('cm doctor', C.CYAN)}")
    else:
        print(f"  적용: {colored('cm setup', C.CYAN)}")


# === CLI Entry ===

# === Unified command registry (single source of truth) ===
# Each entry drives the `cm` CLI dispatch and the generated help text, so a new
# command is registered in exactly one place.
...
def _cmd_app(rest):
    ...
    dry_run = any(arg in ("--dry-run", "-n") for arg in app_args)
    app_args = [
        arg for arg in app_args
        if arg not in APP_RESTART_FLAGS and arg not in ("--dry-run", "-n")
    ]
    launch_app_account(target, app_args, dry_run=dry_run, restart=restart)
...
def auth_doctor():
    ...
    # Whether the app can reach proxy-served models at all, and whether every
    # account home agrees. A stale account config is the usual reason one account
    # sees proxy models and another does not.
    if shutil.which("ocx"):
        target = app_routing_target()
        if target:
            print(f"  앱 라우팅       : {target} {colored('✓', C.GREEN)}")
        else:
            print(f"  앱 라우팅       : 네이티브 (프록시 모델 없음) {colored('!', C.YELLOW)}")
        stale = [
            name for name in list_accounts()
            if (HOMES_DIR / stable_account_key(name) / "config.toml").is_file()
            and ("openai_base_url" in (HOMES_DIR / stable_account_key(name) / "config.toml")
                 .read_text(encoding="utf-8", errors="replace")) != bool(target)
        ]
        if stale:
            print(
                f"  {colored('!', C.YELLOW)} 앱 홈과 라우팅이 다른 계정: {len(stale)}개 — "
                f"{colored('cm setup', C.CYAN)}으로 갱신"
            )

    print_proxy_account_pool()


PROXY_ACCOUNT_POOL_URL = f"{cm_integrations.proxy_base_url()}/api/codex-auth/accounts"
...
def status_payload() -> dict:
    """Machine-readable account status.

    Exists so external tools (a dashboard, a status bar, a script) can consume
    the same data the table shows without scraping ANSI output. Emails are
    included because they are the account identifier the user already sees;
    tokens and other secrets never are.
    """
    ensure_dirs()
    accounts = list_accounts()
    app_context = get_live_app_context(accounts)
...
COMMANDS = [
    {"names": ["status", "list"], "usage": "cm status [--json]",
     "desc": "계정/잔여량 표 출력", "handler": _cmd_status},
    {"names": ["switch", "app-switch", "s"], "usage": "cm switch <번호>",
     "desc": "전역 App 계정 전환(auth.json 교체)", "handler": _cmd_switch},
    {"names": ["usage-proxy", "proxy"], "usage": "cm usage-proxy <번호> <값>",
     "desc": "usage 조회 프록시(URL|direct|system)", "handler": _cmd_proxy},
    {"names": ["quota-debug", "debug-quota"], "usage": "cm quota-debug <번호>",
     "desc": "usage 조회 네트워크/HTTP 진단", "handler": _cmd_quota_debug},
    {"names": ["reset-credits", "reset-credit", "credits", "resets"], "usage": "cm reset-credits [--available-only]",
     "desc": "모든 계정의 초기화권 수·사용기한·남은 시간 조회", "handler": _cmd_reset_credits},
    {"names": ["cli", "launch", "run", "c"], "usage": "cm cli <번호>",
     "desc": "계정별 isolated CLI 실행", "handler": _cmd_cli},
    {"names": ["app", "app-launch", "open-app", "open", "ca", "o"], "usage": "cm app <번호> [--restart|--dry-run]",
     "desc": "계정별 Desktop App 별도 실행(--restart는 해당 계정만 재시작, --dry-run은 실행 경로만 확인)", "handler": _cmd_app},
    {"names": ["import-app", "sync-app", "import"], "usage": "cm import-app [이름] [--dry-run]",
     "desc": "현재 ~/.codex 로그인 가져오기/갱신", "handler": _cmd_import_app},
    {"names": ["refresh", "reauth"], "usage": "cm refresh <번호> [--once|--browser]",
     "desc": "선택 계정 토큰 갱신(기기 코드 지속 대기 기본)", "handler": _cmd_refresh},
    {"names": ["add"], "usage": "cm add [--once|--browser]",
     "desc": "새 계정 추가(기기 코드 지속 대기 기본)", "handler": _cmd_add},
    {"names": ["doctor", "auth-doctor"], "usage": "cm doctor",
     "desc": "인증 경로/로그인 대기/임시 홈 진단", "handler": _cmd_doctor},
    {"names": ["cleanup-temp"], "usage": "cm cleanup-temp [--yes]",
     "desc": "중단된 로그인 임시 홈 확인/명시적 정리", "handler": _cmd_cleanup_temp},
    {"names": ["remove"], "usage": "cm remove",
     "desc": "계정 삭제", "handler": _cmd_remove},
    {"names": ["setup", "init"], "usage": "cm setup [--check]",
     "desc": "필수 구성 점검·opencodex 설치/업그레이드·OCX 모드 확인·기존 로그인 가져오기", "handler": _cmd_setup},
    {"names": ["stack"], "usage": "cm stack [status|apply]",
     "desc": "codex CLI/opencodex 버전·프록시 상태 확인", "handler": _cmd_stack},
    {"names": ["update", "upgrade", "codex-update", "self-update"], "usage": "cm update",
     "desc": "Codex CLI 최신 버전 설치/검증", "handler": _cmd_update},
    {"names": ["help", "-h", "--help"], "usage": "cm help",
     "desc": "도움말", "handler": _cmd_help},
]
...
SYNC_BEFORE_COMMANDS = {
    "status", "switch", "cli", "app", "reset-credits",
}
...
def print_help():
    ...
    print("  여러 ChatGPT/Codex 계정을 한 대에서 관리합니다.")
    print()
    print(colored("  명령", C.BOLD))
    for entry in COMMANDS:
        print(f"    {entry['usage']}")
        print(f"        {entry['desc']}")
    print()
    print(colored("  예시", C.BOLD))
    print("    cm setup                  초기 설정/점검")
    print("    cm status                 계정 표 보기")
    print("    cm switch 2               2번 계정으로 App 전환")
    print("    cm cli 2                  2번 계정으로 CLI 실행")
    print("    cm reset-credits          모든 계정 초기화권/사용기한/남은 시간 보기")
    print()
    print("  · 번호는 표의 No. 선택자는 번호/이메일 일부/ID prefix 모두 가능")
    print()
...
if __name__ == "__main__":
    try:
        ...
        # Ctrl-C at a prompt is a normal way to abort; don't dump a traceback.
        print()
        sys.exit(130)
    except EOFError:
        # No interactive input available (piped stdin, CI, non-TTY shell). The
        # interactive commands need a terminal, so say so instead of showing a
        # traceback from deep inside an input() call.
        print("\n  입력을 받을 수 없습니다. 이 명령은 터미널에서 실행하거나 인자를 직접 지정하세요.")
        sys.exit(2)
