...
def patched_os_name(name: str):
    """Make `codex_multi` see a different `os.name` without touching `os`.

    Patching the shared `os` module also changes what `pathlib` sees, which
    makes it build a PosixPath on Windows (and vice versa) and fail. Swapping in
    a stand-in module keeps the override local to `codex_multi`.
    """
    stub = types.SimpleNamespace(**{attr: getattr(os, attr) for attr in dir(os) if not attr.startswith("__")})
    stub.name = name
    return mock.patch.object(cm, "os", stub)
...
class CodexMultiAuthTests(unittest.TestCase):
    ...
    def test_windows_powershell_prefers_pwsh_7(self):
        with patched_os_name("nt"), mock.patch(
    ...
    def test_posix_app_launch_detaches_from_tui_terminal_session(self):
        ...
        with patched_os_name("posix"), mock.patch.object(
    ...
    def test_explicit_restart_skips_activation_entirely(self):
        ...
        ), mock.patch("codex_multi.subprocess.Popen"):
    ...
    def test_windows_app_launch_keeps_detached_creation_flags(self):
        ...
        with patched_os_name("nt"), mock.patch.object(
