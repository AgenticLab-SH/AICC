...
def create_thread_state(path: Path, rows: list[tuple[str, str]]) -> None:
    connection = sqlite3.connect(path)
    try:
        connection.execute("create table threads (id text primary key, cwd text not null)")
        connection.executemany("insert into threads(id, cwd) values (?, ?)", rows)
        connection.commit()
    finally:
        connection.close()
...
class ThreadIndexTests(unittest.TestCase):
    ...
    def test_project_assignment_backfill_uses_deepest_root_and_preserves_decisions(self):
        database = self.home / "state_5.sqlite"
        create_thread_state(database, [
            ("web", "/workspace/projects/web/app"),
            ("broad", "/workspace/notes"),
            ("kept", "/workspace/projects/web"),
            ("projectless", "/workspace/projects/web"),
            ("outside", "/other/place"),
        ])
        global_state = self.home / index.GLOBAL_STATE_FILE
        index._write_json(global_state, {
            "local-projects": {
                "broad-project": {"rootPaths": ["/workspace"]},
                "web-project": {"rootPaths": ["/workspace/projects/web"]},
            },
            "thread-project-assignments": {
                "kept": {
                    "projectKind": "local",
                    "projectId": "broad-project",
        ...
        result = index._sync_thread_project_assignments(database, global_state)
        saved = index._read_json(global_state)["thread-project-assignments"]

        self.assertEqual(result["added"], 2)
        self.assertEqual(saved["web"]["projectId"], "web-project")
        self.assertEqual(saved["broad"]["projectId"], "broad-project")
        self.assertEqual(saved["kept"]["projectId"], "broad-project")
        self.assertNotIn("projectless", saved)
        self.assertNotIn("outside", saved)

    def test_up_to_date_sync_still_repairs_project_assignments(self):
        database = self.home / "state_5.sqlite"
        create_thread_state(database, [("thread-one", "/workspace/project")])
        index._write_json(self.home / index.GLOBAL_STATE_FILE, {
            "local-projects": {
                "project": {"rootPaths": ["/workspace/project"]},
            }
        })
        fingerprint = index.source_fingerprint(self.home)
        index._write_json(index._state_path(self.manager), {
            "accounts": {
                "account": {"source_sha256": fingerprint["sha256"]},
            }
        })
        ...
        self.assertEqual(result["project_assignments"]["added"], 1)
        saved = index._read_json(self.home / index.GLOBAL_STATE_FILE)
        self.assertEqual(
            saved["thread-project-assignments"]["thread-one"]["projectId"],
            "project",
        )

    def test_status_detects_database_shrink_with_unchanged_rollout_fingerprint(self):
        database = self.home / "state_5.sqlite"
        create_state(database, ["one"])
        fingerprint = index.source_fingerprint(self.home)
        index._write_json(index._state_path(self.manager), {
            "accounts": {
                "account": {
